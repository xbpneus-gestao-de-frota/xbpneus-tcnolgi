...
const RevendaFormPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await axios.put(`/api/revenda/${id}/`, formData, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
        ...
        await axios.post('/api/revenda/', formData, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
        setSuccessMessage('Revenda criada com sucesso!');
      }
      ...
      console.error('Erro ao salvar revenda:', err);
      setError(err.response?.data?.message || 'Erro ao salvar revenda');
    }
  };
...
export default RevendaFormPage;
